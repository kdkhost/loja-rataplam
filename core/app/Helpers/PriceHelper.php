...
class PriceHelper
{
    ...
    {
        $curr = self::activeCurrency();
        $price = self::testPrice($price * $curr->value);
        if (self::currencyDirection() == 1) {
            return self::currencyWithSpacing($curr->sign, $price);
        } else {
    ...
    public static function testPrice($price)
    {
        ...
        $decimalSeparator = self::decimalSeparator($setting);
        $thousandSeparator = self::thousandSeparator($setting);

        if ($setting->is_decimal == 1) {
            if (is_numeric($price) || floor($price) != $price) {
                return number_format($price, 2, $decimalSeparator, $thousandSeparator);
            } else {
                return number_format($price, 2, $decimalSeparator, $thousandSeparator);
            }
        } else {
    ...
    protected static function activeCurrency()
    {
        if (Session::has('currency')) {
            $currency = Currency::where('status', 1)->whereIn('name', ['BRL', 'USD'])->find(Session::get('currency'));
        ...
        return Currency::where('status', 1)->where('is_default', 1)->first()
            ?: Currency::where('status', 1)->whereIn('name', ['BRL', 'USD'])->first()
            ?: Currency::where('is_default', 1)->first();
    }

    protected static function currencyDirection()
    {
        $setting = Setting::first();
        return $setting ? (int) $setting->currency_direction : 1;
    }

    protected static function decimalSeparator($setting)
    {
        return self::isBrazilianLocale() ? ',' : ($setting->decimal_separator ?: '.');
    }

    protected static function thousandSeparator($setting)
    {
        return self::isBrazilianLocale() ? '.' : ($setting->thousand_separator ?: ',');
    }

    protected static function currencyWithSpacing($sign, $price)
    {
        return trim($sign) === 'R$' ? 'R$ ' . $price : $sign . $price;
    }

    protected static function isBrazilianLocale()
    {
        $locale = strtolower(str_replace('-', '_', app()->getLocale()));

        return $locale === 'pt'
            || str_starts_with($locale, 'pt_')
            || str_contains($locale, 'pt_dashboard')
            || str_contains($locale, 'pt_website');
    }
