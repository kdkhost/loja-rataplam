        ...
        <div class="col-lg-8">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @php
                        $licenseNames = is_array($license_name ?? null) ? $license_name : [];
                        $licenseKeys = is_array($license_key ?? null) ? $license_key : [];
                        $licensePairs = [];
                        foreach ($licenseNames as $index => $licenseName) {
                            if ($licenseName !== null && $licenseName !== '') {
                                $licensePairs[] = [
                                    'name' => $licenseName,
                                    'key' => $licenseKeys[$index] ?? '',
                                ];
                            }
                        }
                    @endphp
                    @if(count($licensePairs))
                    @foreach($licensePairs as $license)
                            ...
                            <div class="flex-grow-1">
                                <div class="form-group">
                                    <input type="text" class="form-control"
                                        name="license_name[]"
                                        placeholder="{{ __('License Name') }}" value="{{$license['name']}}">
                                    </div>
                            </div>
                            <div class="flex-grow-1">
                                <div class="form-group">
                                    <input type="text" class="form-control"
                                        name="license_key[]"
                                        placeholder="{{ __('License Key') }}" value="{{$license['key']}}">
                                    </div>
                            </div>
                            <div class="flex-btn">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div id="specifications-section" class="{{ $item->is_specification == 0 ? 'd-none' : '' }}">
                        @php
                            $specificationNames = is_array($specification_name ?? null) ? $specification_name : [];
                            $specificationDescriptions = is_array($specification_description ?? null) ? $specification_description : [];
                            $specificationPairs = [];
                            foreach ($specificationNames as $index => $specificationName) {
                                if ($specificationName !== null && $specificationName !== '') {
                                    $specificationPairs[] = [
                                        'name' => $specificationName,
                                        'description' => $specificationDescriptions[$index] ?? '',
                                    ];
                                }
                            }
                        @endphp
                        @if(count($specificationPairs))
                        @foreach($specificationPairs as $specification)
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                <div class="form-group">
                                    <input type="text" class="form-control"
                                        name="specification_name[]"
                                        placeholder="{{ __('Specification Name') }}" value="{{$specification['name']}}">
                                    </div>
                            </div>
                            <div class="flex-grow-1">
                                <div class="form-group">
                                    <input type="text" class="form-control"
                                        ...
                                        placeholder="{{ __('Specification description') }}" value="{{$specification['description']}}">
                                    </div>
                            </div>
                            <div class="flex-btn">
                                @if($loop->first)
